...
let windowsSandboxWorkspace;
let windowsSandboxQueue = Promise.resolve();
let sandboxSessionCount = 0;
let sandboxCommandCount = 0;
const sandboxCommandWaiters = new Set();
const sessionStates = new WeakMap();
export function createPiSandboxModeRef(value) {
    return { value };
}
export function createPiSandboxExtension(workspace, modeRef) {
    return (pi) => {
...
export function createPiSandboxConfig(workspace) {
...
export async function registerPiSandboxSession(session, workspace, modeRef, writeMode) {
    const state = {
...
export async function updatePiSandboxSession(session, workspace, writeMode) {
...
export async function releasePiSandboxSession(session) {
    const state = sessionStates.get(session);
    if (!state)
        return;
    ...
    if (process.platform === "win32")
        await enqueueWindowsSandbox(reset);
    else
        await reset();
}
function dynamicTool(unrestricted, restricted, modeRef, writeCapable = false) {
    return {
        ...restricted,
        execute: (...args) => {
...
function createReadOperations(workspace) {
    return {
...
function createWriteOperations(workspace) {
    return {
...
function createEditOperations(workspace) {
    return {
        readFile: async (path) => readFile(await assertPiWorkspacePath(path, workspace)),
        writeFile: async (path, content) => writeFile(await assertPiWorkspacePath(path, workspace, true), content),
...
function createGrepOperations(workspace) {
    return {
...
function createFindOperations(workspace) {
    return {
        exists: async (path) => {
            try {
...
function createLsOperations(workspace) {
    return {
        exists: async (path) => {
            try {
                await access(await assertPiWorkspacePath(path, workspace));
...
async function acquirePiSandbox(state) {
    if (state.acquired)
        return;
    state.acquired = true;
...
async function ensureWindowsSandbox(workspace) {
...
async function runSandboxedCommand(command, cwd, options) {
    ...
    const wrapped = await SandboxManager.wrapWithSandboxArgv(command, undefined, process.platform === "win32" ? undefined : customConfig, options.signal, workspace);
    try {
...
function spawnSandboxedCommand(argv, environment, cwd, options) {
...
function createPiSandboxFilesystem(workspace) {
    return {
        denyRead: protectedHomePaths(),
...
async function assertPiWorkspacePath(path, workspace, _forWrite = false) {
    const resolvedWorkspace = resolveWorkspace(workspace);
...
async function resolveExistingBoundary(path) {
    let candidate = path;
    for (;;) {
        try {
            return {
            ...
            if (parent === candidate)
                return { boundaryPath: candidate, suffix: relative(candidate, path) };
            candidate = parent;
        }
    }
}
async function withSandboxCommand(operation) {
    ...
    try {
        ...
        if (sandboxCommandCount === 0) {
            for (const resolveWaiter of sandboxCommandWaiters)
                resolveWaiter();
            sandboxCommandWaiters.clear();
        }
    }
}
function waitForSandboxCommands() {
    if (sandboxCommandCount === 0)
...
function resolveWorkspace(workspace) {
    ...
    if (!existsSync(absolute))
        throw new Error(`Pi workspace does not exist: ${workspace}`);
