...
import { chmodSync, closeSync, linkSync, mkdirSync, openSync, readFileSync, renameSync, rmSync, writeSync, } from "node:fs";
...
export function localAgentDaemonPaths(stateDir, platform = process.platform) {
...
export function ensureLocalAgentDaemonStateDir(stateDir) {
    ...
    acquire() {
        ...
        for (let attempt = 0; attempt < 2; attempt += 1) {
            ...
            try {
                ...
                if (!removeStaleLock(this.paths.lockPath))
                    continue;
            }
        }
...
export function ensureLocalAgentDaemonSecret(paths) {
    ...
    try {
        ...
        if (!isDaemonSecret(existing))
            throw new Error("Local agent daemon secret is invalid.");
        return existing;
    }
}
export function readLocalAgentDaemonSecret(paths) {
    try {
        const secret = readFileSync(paths.secretPath, "utf8").trim();
...
export function removeLocalAgentDaemonFiles(paths) {
    rmSync(paths.pidPath, { force: true });
    if (process.platform !== "win32")
        rmSync(paths.socketPath, { force: true });
}
export function readDaemonPid(pidPath) {
    try {
...
function isDaemonSecret(secret) {
