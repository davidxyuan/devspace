...
- NT1（C02180288）原 checkout 為 detached HEAD `9c4462b` 且有多個未提交 dist/installer 變更，因此未覆蓋。已 fetch 最新 branch 並建立 DevSpace managed worktree `C:\Users\op.yo\.devspace\worktrees\devspace-f86d79ba`，base 精確為 `4d67e32`；independent-task、control-core、supervisor、lifecycle、39 檔 package validation 全部通過。Hermes 曾長時間保持 listener PID 但 health probe 不回、owner/operator 全部 502；以舊 elevated watchdog 已有的 `restart-devspace.flag` 受控 restart 路徑重啟 managed stack 後，owner/operator MCP 恢復。NT1 具 admin 權限，因此舊 `DevSpaceNgrokWatchdogUserPoller` 已真正 Disabled。all-in-one installer 隨後同樣由 Kaspersky Event 4662 明確終止（`0x40000015`）；沿用 transaction manifest 分段建立 `DevSpaceWatchdogSupervisor-01c9557cbf3a`、正式 install record 與 Tray。初次 Hermes/public probe 因忙碌短暫 timeout，未手動 Verify；5 分鐘 backoff 自動 retry 後 Dashboard 回 `GREEN / Healthy`，DevSpace/Hermes/Router/ngrok 與 public MCP 均 Healthy，下一輪 public probe 回到 6 小時節流。Rollback backup：`C:\Users\op.yo\.devspace\configuration-backups\tray-install-20260909-120042-bb47e681`。
...
| 三台舊機升級 | NT2R CTF、FrameStation、NT1 均已用 clean clone / managed worktree + 分段部署完成並實機 `GREEN / Healthy`；三台皆有正式 Supervisor / install record，public probe 維持 6 小時節流 |
