...
| A cmd or PowerShell window flashes every minute | Update the checkout and rerun the installer with `-TaskLauncher PowerShell`, approving UAC so it can register an `S4U` background task. Then confirm the task principal reports `LogonType: S4U`; an older `InteractiveToken` task must be recreated. |
